...
class GestorConfiguracion:
    ...
    def _cargar_configuracion(self):
        ...
        try:
            ...
            if self.archivo_config_principal.exists():
                with open(self.archivo_config_principal, 'r', encoding='utf-8') as f:
                    datos_config = yaml.safe_load(f)
                    self.configuracion_actual = self._dict_a_configuracion(datos_config)
            else:
                # Crear configuración por defecto
                self.configuracion_actual = ConfiguracionEmpresarial()
    ...
    def _cargar_perfiles(self):
        """Cargar perfiles de configuración disponibles."""
        try:
            for archivo_perfil in self.directorio_perfiles.glob("*.yaml"):
                nombre_perfil = archivo_perfil.stem
                with open(archivo_perfil, 'r', encoding='utf-8') as f:
                    datos_perfil = yaml.safe_load(f)
                    self.perfiles_disponibles[nombre_perfil] = self._dict_a_configuracion(datos_perfil)
        except Exception as e:
            logger.error(f"Error cargando perfiles: {e}")
    
    def _dict_a_configuracion(self, datos: Dict[str, Any]) -> ConfiguracionEmpresarial:
        ...
        if 'nombre_organizacion' in datos:
            config.nombre_organizacion = datos['nombre_organizacion']
        if 'entorno' in datos:
            config.entorno = TipoEntorno(datos['entorno'])
        if 'nivel_habilidad' in datos:
            config.nivel_habilidad = NivelHabilidad(datos['nivel_habilidad'])
        
        # Mapear configuraciones específicas
        if 'base_datos' in datos:
            bd_datos = datos['base_datos']
            config.base_datos = ConfiguracionBaseDatos(**bd_datos)
        
        if 'rendimiento' in datos:
            rend_datos = datos['rendimiento']
            config.rendimiento = ConfiguracionRendimiento(**rend_datos)
        
        if 'seguridad' in datos:
            seg_datos = datos['seguridad']
            config.seguridad = ConfiguracionSeguridad(**seg_datos)
        
        if 'analisis' in datos:
            anal_datos = datos['analisis']
            config.analisis = ConfiguracionAnalisis(**anal_datos)
        
        if 'salida' in datos:
            sal_datos = datos['salida']
            config.salida = ConfiguracionSalida(**sal_datos)
    ...
    def _guardar_configuracion(self):
        ...
        try:
            ...
            datos_config = asdict(self.configuracion_actual)
            
            # Convertir enums a strings
            datos_config['entorno'] = self.configuracion_actual.entorno.value
            datos_config['nivel_habilidad'] = self.configuracion_actual.nivel_habilidad.value
            
            # Convertir fechas a strings
            datos_config['creado_en'] = self.configuracion_actual.creado_en.isoformat()
            datos_config['actualizado_en'] = datetime.now().isoformat()
            
            # Guardar como YAML
            with open(self.archivo_config_principal, 'w', encoding='utf-8') as f:
                yaml.dump(datos_config, f, default_flow_style=False, allow_unicode=True, indent=2)
    ...
    def _crear_respaldo(self):
        ...
        try:
            if self.archivo_config_principal.exists():
                ...
                archivo_respaldo = self.directorio_respaldos / f"config_backup_{timestamp}.yaml"
                
                import shutil
                shutil.copy2(self.archivo_config_principal, archivo_respaldo)
                
                # Mantener solo los últimos 10 respaldos
                respaldos = sorted(self.directorio_respaldos.glob("config_backup_*.yaml"))
                if len(respaldos) > 10:
                    for respaldo_viejo in respaldos[:-10]:
                        respaldo_viejo.unlink()
