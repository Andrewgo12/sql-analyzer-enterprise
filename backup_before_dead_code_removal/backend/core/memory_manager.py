...
class MemoryMonitor:
        ...
        while self.monitoring:
            try:
                ...
                if memory_percent >= self.critical_threshold:
                    self._trigger_callbacks(memory_percent, "critical")
                    self._emergency_cleanup()
                elif memory_percent >= self.warning_threshold:
                    self._trigger_callbacks(memory_percent, "warning")
    ...
    def _trigger_callbacks(self, memory_percent: float, level: str):
        """Trigger registered callbacks"""
        for callback in self.callbacks:
            try:
                callback(memory_percent, level)
            except Exception as e:
    ...
    def _perform_cleanup(self):
        ...
        logger.info(f"Memory after cleanup: {memory.percent:.1f}% used")
...
class MemoryManager:
    ...
    def _handle_memory_alert(self, memory_percent: float, level: str):
        """Handle memory alerts"""
        if level == "critical":
            logger.error(f"CRITICAL: Memory usage at {memory_percent:.1f}%")
            self._emergency_cleanup()
        elif level == "warning":
            logger.warning(f"WARNING: Memory usage at {memory_percent:.1f}%")
    ...
    def _optimize_memory(self):
        ...
        # Clear least used pools
        stats = self.pool.get_stats()
        
        # Find pools with low reuse rates
        for obj_type in list(self.pool.pools.keys()):
            created = stats.get(f"{obj_type}_created", 0)
            reused = stats.get(f"{obj_type}_reused", 0)
            
            if created > 0 and reused / created < 0.5:  # Low reuse rate
                self.pool.clear_pool(obj_type)
                logger.info(f"Cleared low-efficiency pool: {obj_type}")
