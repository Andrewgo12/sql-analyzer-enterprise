...
class UploadManager {
    ...
    getFileFromUploadElement(uploadElement) {
        // Extract file from upload element
        if (!uploadElement) return null;

        // Check if element has file data stored
        if (uploadElement.dataset.file) {
            try {
                return JSON.parse(uploadElement.dataset.file);
            } catch (e) {
                if (window.Utils) Utils.error('Error parsing file data:', e);
            }
        }

        // Check if element has file reference
        if (uploadElement._file) {
            return uploadElement._file;
        }

        // Try to find file input within element
        ...
        if (window.Utils) Utils.warn('No file found in upload element');
        return null;
    }
